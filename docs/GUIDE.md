...
Docker files in this tree (`Dockerfile.dev`, `docker-compose.dev.yml`) are **not** part of the lab. Ignore them unless you already use Docker for your own workflow.
...
Deadline **25 September 2026, 23:59** (one week after the last Lab 1 session). Submit **only on Moodle**: the zip after `./gradlew clean`, **with `.git`**. There is no GitHub delivery.
